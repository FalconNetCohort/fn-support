...
export default function InProgress() {
  ...
  useEffect(() => {
    const fetchTickets = async () => {
      ...
      const allTickets = [
        ...featureSnapshot.docs.map((doc) => ({
          ...doc.data(),
          id: doc.id,
          type: "Feature",
        })),
        ...supportSnapshot.docs.map((doc) => ({
          ...doc.data(),
          id: doc.id,
          type: "Support",
        })),
  ...
  const filteredTickets = tickets
    .filter(
      (ticket) =>
        ticket.featureDescription
          ?.toLowerCase()
          .includes(searchTerm.toLowerCase()) ||
        ticket.bugDescription
          ?.toLowerCase()
          .includes(searchTerm.toLowerCase()) ||
        ticket.supplementalInfo
          ?.toLowerCase()
          .includes(searchTerm.toLowerCase())
    )
    .sort((a, b) => {
      ...
      <main className="flex min-h-screen flex-col items-center justify-center p-6">
        ...
        <div className="w-full max-w-5xl">
          ...
          {filteredTickets.map((ticket) => (
            <div
              key={ticket.id}
              className="p-4 mb-4 border rounded-lg shadow bg-gray-50 dark:bg-gray-800"
            >
              <h2 className="text-xl font-semibold">
                {ticket.type}:{" "}
                {ticket.featureDescription || ticket.bugDescription}
              </h2>
              <p className="text-gray-400 mb-2">{ticket.supplementalInfo}</p>
              ...
              {ticket.comments &&
                ticket.comments.map((comment, index) => (
                  <div
                    key={index}
                    className="mt-4 p-2 border-t border-gray-600"
                  >
                    <p className="text-gray-300">
                      <strong>Comment from FalconNet Team:</strong>{" "}
                      {comment.text}{" "}
